...
void carrera(unsigned long int speed) {
	...
	while (1) {
		for (int i = 0; i < 16; ++i) {
			...
			//\n representa un salto de lÌnea, para que la siguiente salida aparezca en una nueva lÌnea.
			mostrar(tabla[i]);
			retardo(speed);
			system("cls"); //limpia la consola
...
void efectoPulso(unsigned long int speed) {
	...
	while (1) {
		printf("Presione ESC para volver al men˙ principal\n");
		printf("Delay: %lu\n", speed);
		...
		//Si es asÌ, la funciÛn return se ejecuta, lo que permite salir del bucle y finalizar la funciÛn.
		if (GetAsyncKeyState(VK_ESCAPE) & 0x0001) {
			return;
		}
		...
		// Incrementar o reiniciar el Ìndice de brillo
