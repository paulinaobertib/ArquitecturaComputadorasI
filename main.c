	...
	for (attempt = 0; attempt < MAX_ATTEMPTS; attempt++) { //ciclo for para los intentos de la password
		...
		while ((c = getch()) != 13) { //13 es el enter en ASCII
			if (c != 8) { //8 es la tecla de borrar en ASCII
				...
				if (j > 0) {
					printf("\b \b"); //Al imprimir "\b \b", se muestra un car锟絚ter de retroceso (\b) para mover el cursor hacia la izquierda, luego se imprime un espacio en blanco para sobrescribir el 锟絣timo car锟絚ter visible y finalmente se emite otro car锟絚ter de retroceso para colocar el cursor nuevamente en la posici锟絥 original.
					j--;
				}
			}
		}
...
void retardo(unsigned long int a){ 
	//"unsigned long int" (representa un n煤mero entero sin signo y generalmente ocupa 4 bytes en la memoria). La funci贸n no devuelve ning煤n valor (void).
	
	//Bucle de a que se ejecuta siempre que a sea distinto de cero
	//En cada iteraci贸n del bucle, se decrementa el valor de "a" en 1
	while (a) a--;
}
...
void carrera(unsigned long int speed) {
	...
	while (1) {
		for (int i = 0; i < 16; ++i) {
			...
			//\n representa un salto de l韓ea, para que la siguiente salida aparezca en una nueva l韓ea.
			mostrar(tabla[i]);
			retardo(speed);
			system("cls"); //limpia la consola
...
void choque(unsigned long int speed) {//Esto define la funci贸n choque que toma un par谩metro "speed" de tipo "unsigned long int".
  // Se define un arreglo llamado "tabla" que contiene una secuencia de valores en formato hexadecimal. 
  //Estos valores representan los patrones de luces que se mostrar谩n durante el efecto de choque.
    ...
    while (1) { //Esto inicia un bucle infinito que controla la ejecuci贸n del efecto de choque. 
				//El bucle se repetir谩 continuamente hasta que se detenga manualmente o se cumpla alguna condici贸n de salida.
        
		for (int i = 0; i < 7; ++i) {
			//itera a trav茅s de los elementos del arreglo tabla. 
			//En cada iteraci贸n, se realiza una serie de acciones relacionadas con la visualizaci贸n del patr贸n de luz correspondiente.
            ...
            retardo(speed); //Llama a una funci贸n retardo. 
            system("cls"); //Limpia la consola para borrar los patrones de luces anteriores y prepararse para mostrar el siguiente patr贸n.
